...
from shop_bot.payments.lavatop import LavaTopConfigError, LavaTopRequestError, LavaTopService
...
def get_user_router() -> Router:
    ...
    async def topup_pay_yookassa(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            if payment_id:
                payment_payload["metadata"]["payment_id"] = payment_id
                try:
                    create_pending_transaction(payment_id, user_id, price_float_for_metadata, dict(payment_payload["metadata"]))
                except Exception as e:
                    logger.warning(f"YooKassa top-up: не удалось создать ожидающую транзакцию {payment_id}: {e}")
            else:
                logger.warning("YooKassa top-up: провайдер не вернул payment_id, идемпотентность webhook будет ограничена.")
        ...
        try:
            ...
            if payment_id:
                ...
                try:
                    ...
                    logger.warning(f"YooKassa: не удалось создать ожидающую транзакцию {payment_id}: {e}")
            else:
                logger.warning("YooKassa: провайдер не вернул payment_id, идемпотентность webhook будет ограничена.")
    ...
    @user_router.callback_query(PaymentProcess.waiting_for_payment_method, F.data == "pay_lavatop")
    async def create_lavatop_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        await callback.answer("Создаю ссылку Lava.top...")
        ...
        base_price = Decimal(str(plan['price']))
        final_price_decimal = base_price
        ...
        if user_data.get('referred_by') and user_data.get('total_spent', 0) == 0:
            ...
            if discount_percentage > 0:
                final_price_decimal = base_price - (base_price * discount_percentage / 100).quantize(Decimal("0.01"))
        ...
        try:
            invoice = await LavaTopService.create_invoice(
        ...
        except LavaTopConfigError as e:
            await callback.message.answer(str(e))
            await state.clear()
            return
        except LavaTopRequestError as e:
            logger.error(f"Lava.top: ошибка создания инвойса: {e}", exc_info=True)
            await callback.message.answer(str(e))
            await state.clear()
            return
        except Exception as e:
            logger.error(f"Lava.top: непредвиденная ошибка создания инвойса: {e}", exc_info=True)
            await callback.message.answer("Не удалось создать ссылку на оплату через Lava.top.")
            await state.clear()
            return

        if not invoice.invoice_id:
            await callback.message.answer("Lava.top не вернул invoice_id, оплата отменена.")
            await state.clear()
            return

        metadata["payment_id"] = invoice.invoice_id
        try:
            create_pending_transaction(invoice.invoice_id, user_id, float(final_price_decimal), metadata)
        except Exception as e:
            logger.warning(f"Lava.top: не удалось создать ожидающую транзакцию {invoice.invoice_id}: {e}")

        await state.clear()
        try:
            await callback.message.edit_text(
                "Нажмите на кнопку ниже для оплаты через Lava.top:",
                reply_markup=keyboards.create_payment_keyboard(invoice.payment_url)
            )
        except TelegramBadRequest:
