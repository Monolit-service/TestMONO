...
ALL_SETTINGS_KEYS = [
    ...
    "heleket_merchant_id", "heleket_api_key", "lava_api_key", "lava_api_url",
    "lava_env", "lava_timeout_sec", "lava_max_retries", "lava_offer_id_1_month",
    "lava_offer_id_3_months", "lava_offer_id_6_months", "lava_offer_id_12_months",
...
def create_webhook_app(bot_controller_instance):
    ...
    webhook_queue: Queue[WebhookTask] = Queue(maxsize=WEBHOOK_QUEUE_MAXSIZE)

    def _submit_payment_processing(metadata: dict, source: str) -> None:
        if not metadata:
            logger.warning(f"{source}: пустые метаданные платежа, обработка пропущена.")
            return
        ...
        if not bot or not loop or not loop.is_running():
            logger.error(f"{source}: бот или event loop недоступны, webhook не обработан.")
            return
        ...
        def _log_result(done_future):
            try:
                ...
                logger.error(f"{source}: ошибка фоновой обработки платежа: {exc}", exc_info=True)

        future.add_done_callback(_log_result)

    def _parse_json_payload(payload: str, source: str) -> dict | None:
        try:
            data = json.loads(payload)
        except json.JSONDecodeError as exc:
            logger.warning(f"{source}: получен невалидный JSON: {exc}")
            return None
        if not isinstance(data, dict):
            logger.warning(f"{source}: JSON верхнего уровня должен быть объектом.")
            return None
        return data

    def _parse_cryptobot_metadata(payload_string: str) -> dict | None:
        if not payload_string:
            logger.warning("CryptoBot webhook: payload пустой.")
            return None

        parts = payload_string.split(':')
        if len(parts) < 9:
            logger.error(f"CryptoBot webhook: некорректный формат payload: {payload_string}")
            return None
    ...
    def _process_yookassa_webhook(payload: str) -> None:
        event_json = _parse_json_payload(payload, "YooKassa webhook")
        if not event_json or event_json.get("event") != "payment.succeeded":
            return

        payment_object = event_json.get("object") or {}
        payment_id = str(payment_object.get("id") or "").strip()
        amount_info = payment_object.get("amount") or {}

        amount_rub = None
        if amount_info.get("value") is not None:
            try:
                amount_rub = float(amount_info.get("value"))
            except (TypeError, ValueError):
                logger.warning(f"YooKassa webhook: не удалось разобрать сумму для payment_id={payment_id}")
        ...
        if payment_id:
            metadata = database.find_and_complete_pending_transaction(
                ...
                currency_name=(amount_info.get("currency") or "RUB"),
                amount_currency=amount_rub,
            )

        if not metadata:
            raw_metadata = payment_object.get("metadata") or {}
            if isinstance(raw_metadata, dict) and raw_metadata:
                metadata = dict(raw_metadata)
                if payment_id and not metadata.get("payment_id"):
                    metadata["payment_id"] = payment_id

        if not metadata:
            logger.warning(f"YooKassa webhook: метаданные не найдены для payment_id={payment_id}")
            return

        _submit_payment_processing(metadata, "YooKassa webhook")

    def _process_cryptobot_webhook(payload: str) -> None:
        request_data = _parse_json_payload(payload, "CryptoBot webhook")
        if not request_data or request_data.get('update_type') != 'invoice_paid':
            return

        payload_data = request_data.get('payload', {}) or {}
        metadata = _parse_cryptobot_metadata(payload_data.get('payload'))
        if metadata:
            _submit_payment_processing(metadata, "CryptoBot webhook")

    def _process_heleket_webhook(payload: str) -> None:
        data = _parse_json_payload(payload, "Heleket webhook")
        if not data:
            return

        api_key = get_setting("heleket_api_key")
        if not api_key:
            logger.error("Heleket webhook: не задан heleket_api_key.")
            return

        data_for_sign = dict(data)
        sign = data_for_sign.pop("sign", None)
        if not sign:
            logger.warning("Heleket webhook: отсутствует подпись.")
            return

        sorted_data_str = json.dumps(data_for_sign, sort_keys=True, separators=(",", ":"))
        ...
        if data_for_sign.get('status') not in ["paid", "paid_over"]:
            return

        metadata_str = data_for_sign.get('description')
        if not metadata_str:
            logger.warning("Heleket webhook: отсутствует description с метаданными.")
            return

        try:
            metadata = json.loads(metadata_str)
        except json.JSONDecodeError as exc:
            logger.error(f"Heleket webhook: description содержит невалидный JSON: {exc}")
            return

        if not isinstance(metadata, dict):
            logger.warning("Heleket webhook: description должен содержать JSON-объект.")
            return

        _submit_payment_processing(metadata, "Heleket webhook")

    def _process_lava_webhook(payload: str) -> None:
        try:
            from shop_bot.payments.lavatop import LavaTopRequestError, LavaTopService

            payment_id, _ = LavaTopService.parse_success_webhook(payload)
            if not payment_id:
                return
        except LavaTopRequestError as exc:
            logger.warning(f"Lava webhook: ошибка разбора: {exc}")
            return
        except Exception as exc:
            logger.error(f"Lava webhook: внутренняя ошибка разбора: {exc}", exc_info=True)
            return
        ...
        if not metadata:
            logger.warning(f"Lava webhook: pending-транзакция не найдена для payment_id={payment_id}")
            return

        _submit_payment_processing(metadata, "Lava webhook")

    def _process_ton_webhook(payload: str) -> None:
        data = _parse_json_payload(payload, "TON webhook")
        if not data or 'tx_id' not in data:
            return

        txs = (data.get('in_progress_txs') or []) + (data.get('txs') or [])
        for tx in txs:
            in_msg = tx.get('in_msg') or {}
            payment_id = in_msg.get('decoded_comment')
            if not payment_id:
                continue
            ...
            try:
                ...
                logger.warning(f"TON webhook: не удалось разобрать сумму для payment_id={payment_id}")
                continue

            amount_ton = float(amount_nano / 1_000_000_000)
            metadata = find_and_complete_ton_transaction(payment_id, amount_ton)
            if metadata:
                _submit_payment_processing(metadata, "TON webhook")

    def _process_webhook_task(task: WebhookTask) -> None:
        if task.source == "yookassa":
            _process_yookassa_webhook(task.payload)
        elif task.source == "cryptobot":
            _process_cryptobot_webhook(task.payload)
        elif task.source == "heleket":
            _process_heleket_webhook(task.payload)
        elif task.source == "lava":
            _process_lava_webhook(task.payload)
        elif task.source == "ton":
            _process_ton_webhook(task.payload)
        else:
            logger.warning(f"Неизвестный источник webhook: {task.source}")

    def _webhook_worker() -> None:
        while True:
            try:
                task = webhook_queue.get(timeout=1.0)
            except Empty:
                continue
            try:
                _process_webhook_task(task)
            except Exception as exc:
                logger.error(f"Ошибка фонового воркера webhook {task.source}: {exc}", exc_info=True)
            finally:
                webhook_queue.task_done()

    for idx in range(WEBHOOK_WORKERS):
        threading.Thread(
            target=_webhook_worker,
            name=f"webhook-worker-{idx + 1}",
            daemon=True,
        ).start()

    def _accept_webhook(source: str):
        ...
        try:
            webhook_queue.put_nowait(WebhookTask(source=source, payload=payload))
        except Full:
            logger.warning(f"{source}: очередь webhook переполнена.")
    ...
    def yookassa_webhook_handler():
        return _accept_webhook("yookassa")
        try:
    ...
    def cryptobot_webhook_handler():
        return _accept_webhook("cryptobot")
        try:
    ...
    def heleket_webhook_handler():
        return _accept_webhook("heleket")
        try:
            data = request.json
